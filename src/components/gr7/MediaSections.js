...
import { ArrowRight, Play, X, Instagram as InstagramIcon, Grid3x3, } from "lucide-react";
import { MediaSlot, PhoneFrame, NotebookFrame } from "./MediaSlot";
import gr7LogoDark from "@/assets/gr7-logo-dark.png";
import { projects, reels, stories, arts, identity, cinematic, backstage, clientLogos, cases, dashboards, videoTestimonials, instagram, inAction, } from "./mediaConfig";
import { projectDataset } from "./projectData";
...
function Reveal({ children, delay = 0, y = 24, className = "", variant = "rise", }) {
    ...
    return (<motion.div ref={ref} initial={hidden} animate={inView ? v.shown : hidden} transition={{ duration: 0.85, delay, ease: [0.22, 1, 0.36, 1] }} style={{ willChange: "transform, opacity, filter" }} className={className}>
      {children}
    </motion.div>);
}
/* ------------------------------------------------------------------ */
...
function SectionHead({ kicker, title, lead, align = "left", }) {
    return (<div className={align === "center" ? "mx-auto max-w-3xl text-center" : "max-w-3xl"}>
      ...
      {lead && (<p className="mt-6 max-w-2xl text-base text-white/60 md:text-lg">{lead}</p>)}
    </div>);
}
/* ================================================================== */
...
export function ProjectsGrid() {
    ...
    const smallItems = projects
        .map((p, i) => ({ p, i, data: projectDataset[i] }))
        .filter(({ i }) => i % 5 !== 0);
    return (<section id="projetos" className="relative py-32 md:py-40">
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        <SectionHead kicker="Portfólio" title={<>
              Alguns projetos <br />
        ...
        <div className="mt-16 hidden grid-cols-1 gap-5 lg:grid lg:grid-cols-2">
          ...
          <div className="grid grid-cols-1 gap-5">
            {largeItems.map(({ p, i, data }) => (<div key={i} style={{ aspectRatio: "1 / 1" }}>
                <button type="button" onClick={() => data && setActive(data)} className="group relative h-full w-full overflow-hidden rounded-2xl border border-white/10 bg-[#0a0a0a] text-left">
                  <MediaSlot src={p.src} kind={p.kind} alt={`${p.client} — ${p.category}`} className={`absolute inset-0 transition-transform duration-[900ms] ease-out group-hover:scale-110 ${p.bg === "white" ? "object-contain p-8" : "object-cover"}`} label={p.category} bg={p.bg}/>
          ...
          <div className="grid grid-cols-2 gap-5">
            {smallItems.map(({ p, i, data }) => (<div key={i} style={{ aspectRatio: "4 / 5" }}>
                <button type="button" onClick={() => data && setActive(data)} className="group relative h-full w-full overflow-hidden rounded-2xl border border-white/10 bg-[#0a0a0a] text-left">
                  <MediaSlot src={p.src} kind={p.kind} alt={`${p.client} — ${p.category}`} className={`absolute inset-0 transition-transform duration-[900ms] ease-out group-hover:scale-110 ${p.bg === "white" ? "object-contain p-8" : "object-cover"}`} label={p.category} bg={p.bg}/>
                  <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/20 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100"/>
                  <div className="absolute inset-x-0 bottom-0 z-10 translate-y-6 p-6 opacity-0 transition-all duration-500 group-hover:translate-y-0 group-hover:opacity-100">
        ...
        <div className="mt-16 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:hidden">
          {projects.map((p, i) => {
            ...
            const isLarge = i % 5 === 0;
            return (<div key={i} className={`${isLarge ? "sm:col-span-2" : ""}`} style={{ aspectRatio: isLarge ? "1 / 1" : "4 / 5" }}>
                <button type="button" onClick={() => data && setActive(data)} className="group relative h-full w-full overflow-hidden rounded-2xl border border-white/10 bg-[#0a0a0a] text-left">
                  <MediaSlot src={p.src} kind={p.kind} alt={`${p.client} — ${p.category}`} className={`absolute inset-0 transition-transform duration-[900ms] ease-out group-hover:scale-110 ${projects[i]?.bg === "white" ? "object-contain p-8" : "object-cover"}`} label={p.category} bg={projects[i]?.bg}/>
                  <div className="absolute inset-0 bg-gradient-to-t from-black/85 via-black/20 to-transparent opacity-0 transition-opacity duration-500 group-hover:opacity-100"/>
                  <div className="absolute inset-x-0 bottom-0 z-10 translate-y-6 p-6 opacity-0 transition-all duration-500 group-hover:translate-y-0 group-hover:opacity-100">
                    <div className="text-[10px] uppercase tracking-[0.3em] text-white/70">
                      {p.category}
...
export function ReelsSection() {
    ...
    return (<section id="reels" ref={containerRef} onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} className="relative overflow-hidden py-32 md:py-48">
      ...
      <div className="mx-auto max-w-7xl px-6 md:px-10 relative z-10">
        ...
        <div className="relative mt-20 flex h-[600px] items-center justify-center md:h-[800px]">
          {reels.map((r, i) => {
            ...
            return (<motion.div key={i} initial={{ opacity: 0, y: 100, scale: 0.8, filter: "blur(10px)" }} whileInView={{ opacity: 1, y: 0, scale: pos.scale, filter: "blur(0px)" }} viewport={{ once: true, margin: "-100px" }} transition={{
                    duration: 1.2,
                    delay: pos.delay,
                    ease: [0.22, 1, 0.36, 1]
                }} onMouseEnter={() => setHoveredIdx(i)} style={{
                    position: "absolute",
                    left: pos.left,
                    top: pos.top || "50%",
                    translateX: "-50%",
                ...
                }} className={`transition-all duration-700 ${hoveredIdx !== null && hoveredIdx !== i
                ...
                <PhoneFrame active={hoveredIdx === i}>
                  <div className="group relative h-full w-full">
                    <MediaSlot src={r.src} poster={r.poster} kind="video" className="absolute inset-0 object-cover" icon="reel"/>
          ...
          <div className="flex w-full gap-6 overflow-x-auto px-6 pb-12 snap-x snap-mandatory md:hidden [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {reels.map((r, i) => (<motion.div key={`mobile-${i}`} initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="w-[280px] shrink-0 snap-center">
                ...
                <PhoneFrame>
                  <MediaSlot src={r.src} poster={r.poster} kind="video" className="absolute inset-0" icon="reel"/>
                </PhoneFrame>
              </motion.div>))}
          </div>
        </div>
...
export function StoriesRow() {
    ...
    return (<section id="stories" className="relative py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        <SectionHead kicker="Stories" title={<>
        ...
        <div className="mt-12 flex gap-6 overflow-x-auto pb-4 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {stories.map((s, i) => (<button key={i} onClick={() => setOpen(i)} className="group flex shrink-0 flex-col items-center gap-3">
              <div className="relative rounded-full bg-gradient-to-tr from-[#ff1a1a] via-[#ff6b6b] to-[#ff1a1a] p-[3px] transition-transform duration-300 group-hover:scale-105">
                <div className="rounded-full bg-[#0a0a0a] p-[3px]">
                  <div className="relative h-20 w-20 overflow-hidden rounded-full md:h-24 md:w-24">
                    <MediaSlot src={s.src} alt={s.title} className="absolute inset-0" ornate={false} label=""/>
                  </div>
                </div>
              </div>
      ...
      <AnimatePresence>
        {open !== null && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setOpen(null)} className="fixed inset-0 z-[120] flex items-center justify-center bg-[#0a0a0a]/80 backdrop-blur-xl">
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} transition={{ type: "spring", damping: 24, stiffness: 220 }} className="relative aspect-[9/16] h-[80vh] max-h-[720px] overflow-hidden rounded-3xl border border-white/10 bg-neutral-900" onClick={(e) => e.stopPropagation()}>
              <MediaSlot src={stories[open].src} kind={stories[open].kind} alt={stories[open].title} className="absolute inset-0" icon="reel" label={stories[open].title}/>
              <div className="absolute inset-x-0 top-0 flex gap-1 p-3">
                {stories.map((_, k) => (<div key={k} className={`h-0.5 flex-1 rounded-full ${k === open ? "bg-[#0a0a0a]" : "bg-[#0a0a0a]/30"}`}/>))}
              </div>
              <button onClick={() => setOpen(null)} className="absolute right-3 top-6 rounded-full bg-[#0a0a0a]/10 p-2 text-white backdrop-blur">
                <X className="h-4 w-4"/>
              </button>
...
export function ArtsMasonry() {
    ...
    return (<section id="artes" className="relative py-32 md:py-40">
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        <SectionHead kicker="Artes" title={<>
        ...
        <div className="mt-16 columns-2 gap-4 md:columns-3 lg:columns-4">
          {arts.map((a, i) => (<motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.4, delay: (i % 4) * 0.05 }} className={`group relative mb-4 break-inside-avoid overflow-hidden rounded-xl border border-white/10 bg-[#0a0a0a]`}>
              <div className={`relative ${aspects[a.aspect]}`}>
                <MediaSlot src={a.src} alt={a.label} className="absolute inset-0 transition-transform duration-700 group-hover:scale-110" label={a.label}/>
...
export function BrandIdentity() {
    return (<section id="identidade" className="relative py-32 md:py-40">
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        <SectionHead kicker="Identidade Visual" title={<>
              Antes → depois. <br />
        ...
        <div className="mt-16 grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-[#0a0a0a] p-8">
            ...
            <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-neutral-100 grayscale">
              <MediaSlot src={identity.before.src} alt={identity.before.alt} className="absolute inset-0" label="Logo antiga"/>
            </div>
          </div>
          <div className="relative overflow-hidden rounded-3xl border border-[#ff1a1a]/30 bg-gradient-to-br from-white to-[#ff1a1a]/[0.04] p-8 shadow-[0_30px_80px_-40px_rgba(255,26,26,0.4)]">
            ...
            <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-neutral-50">
              <MediaSlot src={identity.after.src} alt={identity.after.alt} className="absolute inset-0" label="Logo GR7"/>
            </div>
          </div>
        </div>
        ...
        <div className="mt-6 grid grid-cols-2 gap-6 md:grid-cols-4">
          {identity.brandbook.map((m, i) => (<div key={i} className="group relative aspect-[3/4] overflow-hidden rounded-2xl border border-white/10 bg-[#0a0a0a]">
              <MediaSlot src={m.src} alt={m.alt} className="absolute inset-0 transition-transform duration-700 group-hover:scale-105" label={m.alt}/>
            </div>))}
        </div>
        ...
        <div className="mt-6 grid grid-cols-1 gap-6 md:grid-cols-2">
          <div className="rounded-3xl border border-white/10 bg-[#0a0a0a] p-8">
            ...
            <div className="flex gap-3">
              {identity.palette.map((c) => (<div key={c} className="flex-1">
...
export function CinematicVideo() {
    ...
    return (<section id="cinema" className="relative py-24 md:py-32">
      <div className="mx-auto max-w-[1400px] px-4 md:px-8">
        <div className="relative aspect-[16/9] w-full overflow-hidden rounded-3xl border border-white/10 bg-neutral-900 shadow-[0_60px_120px_-40px_rgba(0,0,0,0.5)]">
          {cinematic.src ? (<video ref={videoRef} src={cinematic.src} poster={cinematic.poster} muted={!playing} loop playsInline autoPlay className="absolute inset-0 h-full w-full object-cover"/>) : (<MediaSlot src={cinematic.poster} alt={cinematic.alt} className="absolute inset-0" icon="video" label="Reel institucional"/>)}
...
export function BackstageGrid() {
    ...
    return (<section id="bastidores" className="relative py-32 md:py-40">
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        <SectionHead kicker="Bastidores" title={<>
              Dia a dia dentro <br />
        ...
        <div className="mt-16 grid auto-rows-[140px] grid-cols-6 gap-4 md:auto-rows-[180px] md:grid-cols-8">
          {backstage.map((b, i) => (<motion.div key={i} initial={{ opacity: 0, scale: 0.96 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.5, delay: i * 0.05 }} className={`group relative overflow-hidden rounded-2xl border border-white/10 bg-[#0a0a0a] ${spans[b.span]}`}>
              <MediaSlot src={b.src} kind={b.kind} alt={b.label} className="absolute inset-0 transition-transform duration-700 group-hover:scale-110" label={b.label}/>
...
export function ClientsMarquee() {
    ...
    return (<section id="clientes" className="relative overflow-hidden py-24 md:py-32">
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        <div className="text-center">
      ...
      <div className="relative mt-16 [mask-image:linear-gradient(90deg,transparent,black_10%,black_90%,transparent)]">
        <motion.div className="flex gap-8" animate={{ x: ["0%", "-50%"] }} transition={{ duration: 40, ease: "linear", repeat: Infinity }}>
          {row.map((c, i) => (<div key={i} className="flex h-24 w-56 shrink-0 items-center justify-center rounded-2xl border border-white/10 bg-[#0a0a0a]">
              {c.src ? (<img src={c.src} alt={c.name} className="max-h-12 max-w-[70%] object-contain"/>) : (<span className="font-display text-xl uppercase tracking-widest text-white/60">
                  {c.name}
                </span>)}
            </div>))}
        </motion.div>
...
export function CasesShowcase() {
    return (<section id="cases" className="relative py-32 md:py-40">
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        <SectionHead kicker="Cases" title={<>
              Cases completos. <br />
              <span className="italic text-white/50">Problema, estratégia, resultado.</span>
            </>}/>

        <div className="mt-16 space-y-8">
          {cases.map((c, i) => (<motion.article key={i} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }} transition={{ duration: 0.6 }} className={`grid grid-cols-1 gap-8 rounded-3xl border border-white/10 bg-[#0a0a0a] p-6 md:p-10 lg:grid-cols-12 ${i % 2 === 1 ? "lg:[&>*:first-child]:order-2" : ""}`}>
              <div className="relative aspect-[4/3] overflow-hidden rounded-2xl bg-neutral-100 lg:col-span-6">
                <MediaSlot src={c.src} kind={c.kind} alt={c.client} className="absolute inset-0" label={c.client}/>
              </div>
              <div className="flex flex-col justify-between lg:col-span-6">
                ...
                <a href={c.href ?? "#"} className="mt-8 inline-flex w-fit items-center gap-2 rounded-full bg-[#0a0a0a] px-6 py-3 text-sm font-semibold text-white transition hover:bg-[#ff1a1a]">
...
export function DashboardsSection() {
    return (<section id="dashboards" className="relative py-32 md:py-40">
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        <SectionHead kicker="Dashboards" title={<>
              Resultados reais <br />
        ...
        <div className="mt-16 grid grid-cols-1 gap-10 md:grid-cols-2">
          {dashboards.map((d, i) => (<motion.div key={i} initial={{ opacity: 0, y: 40 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.5, delay: i * 0.1 }}>
              <NotebookFrame>
                <MediaSlot src={d.src} kind={d.kind} alt={d.label} className="absolute inset-0" label={d.label}/>
              </NotebookFrame>
...
export function VideoTestimonialsSection() {
    return (<section id="depoimentos-video" className="relative py-32 md:py-40">
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        <SectionHead kicker="Depoimentos" title={<>
      ...
      <div className="mt-16 flex gap-6 overflow-x-auto px-6 pb-6 md:px-10 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {videoTestimonials.map((v, i) => (<motion.a key={i} href={v.href} target="_blank" rel="noopener noreferrer" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-60px" }} transition={{ duration: 0.5, delay: i * 0.08 }} className="group relative aspect-[9/16] w-[280px] shrink-0 overflow-hidden rounded-3xl border border-white/10 bg-neutral-900">
            <MediaSlot src={v.src} kind="image" alt={`${v.name} — ${v.company}`} className="absolute inset-0 transition-transform duration-700 group-hover:scale-105" icon="reel" label="Depoimento"/>
...
export function InstagramProfile() {
    return (<section id="instagram" className="relative py-32 md:py-40">
        ...
        <div className="overflow-hidden rounded-3xl border border-white/10 bg-[#0a0a0a]">
          ...
          <div className="p-6 md:p-10">
            <div className="flex flex-col items-center gap-6 md:flex-row md:items-start md:gap-12">
              <div className="rounded-full bg-gradient-to-tr from-[#ff1a1a] via-[#ff6b6b] to-[#ff1a1a] p-1">
                <div className="rounded-full bg-white p-1">
                  <div className="relative h-28 w-28 overflow-hidden rounded-full bg-white md:h-32 md:w-32">
                    <img src={instagram.avatar.src || gr7LogoDark} alt={instagram.avatar.alt} className="absolute inset-0 h-full w-full object-contain p-3"/>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 text-center md:text-left">
                ...
                <div className="mt-4 max-w-md space-y-1 text-sm text-white/80">
                  ...
                  {instagram.bio.map((line, i) => (<div key={i}>{line}</div>))}
                </div>
              </div>
            </div>
            ...
            <div className="mt-4 grid grid-cols-3 gap-1 md:gap-2">
              {instagram.posts.map((p, i) => (<a key={i} href={p.href} target="_blank" rel="noopener noreferrer" className="group relative aspect-[9/16] overflow-hidden bg-neutral-100">
                  <MediaSlot src={p.src} className="absolute inset-0 transition-transform duration-700 group-hover:scale-110" label="" ornate={false}/>
                </a>))}
            </div>
          </div>
        </div>
...
export function GR7InAction() {
    return (<section id="gr7-em-acao" className="relative py-32 md:py-40">
      <div className="mx-auto max-w-7xl px-6 md:px-10">
        <SectionHead kicker="A GR7 em ação" title={<>
              Tudo, ao mesmo tempo, <br />
              <span className="italic text-white/50">funcionando.</span>
            </>} lead="Vídeos, fotos, artes, reels, dashboards, logos e clientes em uma única montagem. Preparada para receber cada mídia real." align="center"/>
        ...
        <div className="mt-16 grid grid-cols-6 gap-3 md:gap-4">
          {inAction.map((m, i) => {
            ...
            return (<motion.div key={i} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true, margin: "-40px" }} transition={{ duration: 0.5, delay: i * 0.04 }} className={`group relative overflow-hidden rounded-2xl border border-white/10 bg-[#0a0a0a] ${patterns[i]}`}>
                <MediaSlot src={m.src} kind={m.kind} className="absolute inset-0 transition-transform duration-[900ms] group-hover:scale-110" label={`Slot ${i + 1}`}/>
